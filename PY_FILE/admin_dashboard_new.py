...
class Ui_admin_dashboard(object):
    ...
    def setupUi(self, admin_dashboard):
        ...
        # self.bg_table_vieew = QtWidgets.QTableView(self.tab_preview)•
        ...
        self.lbl_dept_txt_2.setGeometry(QtCore.QRect(35, 30, 185, 21))
        sizePolicy = QtWidgets.QSizePolicy(
            QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        ...
        self.lbl_sem_2.setGeometry(QtCore.QRect(470, 25, 160, 31))
        sizePolicy = QtWidgets.QSizePolicy(
            QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
    ...
    def records_it(self):
        try:
                print("hi")
                mycursor = self.mydb.cursor()
                query = ("SELECT * FROM it_exel_file")
                mycursor.execute(query)
                result = mycursor.fetchall()
                query = "DESCRIBE it_exel_file"
                mycursor.execute(query)
                header = [row[0] for row in mycursor.fetchall()]

                self.bg_table_records.setRowCount(len(result))
                self.bg_table_records.setColumnCount(len(result[0]))
                self.bg_table_records.setHorizontalHeaderLabels(header)
                self.bg_table_records.setVerticalHeaderLabels([str(i) for i in range(1, len(result)+1)])

                for i, row in enumerate(result):
                    for j, cell in enumerate(row):
                        self.bg_table_records.setItem(i, j, QtWidgets.QTableWidgetItem(str(cell)))
    ...
    def records_cs(self):
        try:
                print("hi")
                mycursor = self.mydb.cursor()
                query = ("SELECT * FROM cs_exel_file")
                mycursor.execute(query)
                result = mycursor.fetchall()
                query = "DESCRIBE cs_exel_file"
                mycursor.execute(query)
                header = [row[0] for row in mycursor.fetchall()]

                self.bg_table_records.setRowCount(len(result))
                if len(result)==0:
                    self.message("WARNING!!","NO DATA FOUND!!")
                self.bg_table_records.setColumnCount(len(result[0]))
                self.bg_table_records.setHorizontalHeaderLabels(header)
                self.bg_table_records.setVerticalHeaderLabels([str(i) for i in range(1, len(result)+1)])
    ...
    def records_extc(self):
        try:
                print("hi")
                mycursor = self.mydb.cursor()
                query = ("SELECT * FROM extc_exel_file")
                mycursor.execute(query)
                result = mycursor.fetchall()
                query = "DESCRIBE extc_exel_file"
                mycursor.execute(query)
                header = [row[0] for row in mycursor.fetchall()]
    ...
    def records_mechanical(self):
        try:
                print("hi")
                mycursor = self.mydb.cursor()
                query = ("SELECT * FROM mechanical_exel_file")
                mycursor.execute(query)
                result = mycursor.fetchall()
                query = "DESCRIBE mechanical_exel_file"
                mycursor.execute(query)
                header = [row[0] for row in mycursor.fetchall()]
    ...
    def records_bio_mid(self):
        try:
                print("hi")
                mycursor = self.mydb.cursor()
                query = ("SELECT * FROM biomed_exel_file")
                mycursor.execute(query)
                result = mycursor.fetchall()
                query = "DESCRIBE biomed_exel_file"
                mycursor.execute(query)
                header = [row[0] for row in mycursor.fetchall()]
    ...
    def get_record(self):
        ...
        if department_data=="INFORMATION TECHNOLOGY":
            self.records_it()
        elif department_data=="COMPUTER SCIENCE":
            self.records_cs()
        elif department_data=="EXTC":
            self.records_extc()
        elif department_data=="MECHANICAL":
            self.records_mechanical()
        elif department_data=="BIO-MEDICAL":
            self.records_bio_mid()
        else:
            self.message("WARNING!!","PLEASE SELECT OPTIONS!!")
